...
class DataHelper:
    def __init__(self,
        ...
        :param width:
        :param channels:
        :param histogram_equalization:
        """
        # data dictionaries, x for data, y for labels, z for original paths
        self.x = {}
        self.y = {}
        # self.z = {}
    ...
    def get_label_str_from_code(self, code):
        for label, _code in self.labels.items():
            if code == _code:
                return label
    ...
    def process_image(self, image_path: str) -> np.ndarray:
        ...
        # in case the image is in greyscale, build an array with channels dims
        # http://me.umn.edu/courses/me5286/vision/Notes/2015/ME5286-Lecture3.pdf

        if image.shape[2] == 1:
            print('{} is in greyscale, changed dimensions.'.format(image_path))
    ...
    def get_images_dir(self, directory: str):
        """
        """
        shape = '_eq' if self.histogram_equalization else ''
        shape += '({},{},{})'.format(self.height, self.width, self.channels)
    ...
    def get_images(self):
        ...
        """
        for directory in self.directories:
            self.get_images_dir(directory)

    def show_image(self, directory, label, index):
        # get original image
        path = self.paths[directory] + label
        images = [glob(path + e) for e in self.image_extensions]
        images = [item for sublist in images for item in sublist]

        original_img = cv2.imread(images[index])
        original_img = cv2.resize(original_img, (self.height, self.width))

        # plot
        fig, axes = plt.subplots(1, 2, figsize=(12, 4))
        # saved image
        axes[0].imshow(self.x[directory][index])
        axes[0].set_title('Saved image')
        # original image
...
if __name__ == "__main__":
    ROOT_PATH = \
        '/home/hippolyte/Documents/Machine Learning/medical_image_recognition/'
    ...
    for directory in dataclass.directories:
        # get a random index
        ind = np.random.randint(dataclass.x[directory].shape[0])

        # plot the image with label
        plt.imshow(dataclass.x[directory][ind])
        img_label = np.argmax(dataclass.y[directory][ind])
